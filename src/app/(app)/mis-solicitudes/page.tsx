...
export default async function MisSolicitudes() {
  ...
  const orgLabel = ctx.primary_organism?.short_name ?? ctx.primary_organism?.name ?? "—";
  const enProceso = solicitudes.filter((s) =>
    [...ESTADO_GROUPS.pendientes, ...ESTADO_GROUPS.en_proceso].includes(
      s.status as EstadoSolicitud
    )
  ).length;
  const completadas = solicitudes.filter((s) => s.status === "closed").length;
    ...
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <p className="text-3xl font-bold text-amber-500 tabular-nums">{enProceso}</p>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <p className="text-3xl font-bold text-emerald-600 tabular-nums">{completadas}</p>
          <p className="text-xs text-muted-foreground mt-1">pagadas y cerradas</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-5">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-2">
            Total
      ...
      <div className="space-y-2.5">
        {solicitudes.length === 0 ? (
          <div className="bg-card border border-border rounded-xl py-16 text-center">
            ...
            <Link
              href="/solicitudes/nueva"
              className="mt-3 inline-flex text-sm text-primary font-medium hover:underline"
            >
